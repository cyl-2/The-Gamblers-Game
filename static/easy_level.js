...
function draw() {
    ...
    // If player presses two movement keys at one time, they can't move - forces player to press one key at any one time
    /*if ((moveLeft && moveDown) || (moveLeft && moveRight) || (moveLeft && moveUp) || (moveDown && moveUp) || (moveDown && moveRight) || (moveUp && moveRight)) {
        player.xChange = 0;
        player.yChange = 0;
    }*/
...
let coins = setInterval(rich, 1000);
...
function win() {
    ...
    xhttp.open("POST", "https://cs1.ucc.ie/~cyl1/cgi-bin/ca2/run.py/store_score_easylvl", true);
    xhttp.send(data);
